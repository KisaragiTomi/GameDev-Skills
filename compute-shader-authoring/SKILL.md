...
## Godot RenderingDevice Test Runs

Do not use `--headless` as the validation path for Godot `RenderingDevice`,
compute shader, Vulkan, storage buffer, GPU readback, or RenderDoc work.
Headless runs often do not initialize a real rendering backend, so
`RenderingServer.create_local_rendering_device()` or
`RenderingServer.get_rendering_device()` may return `null`.

Use a non-headless Vulkan run for GPU validation:

```powershell
godot --path . --rendering-driver vulkan --script tools/test_x.gd
```

If a quick headless smoke test is still useful, GPU subtests must explicitly
`SKIP` or fail when no `RenderingDevice` exists. Do not add a CPU fallback and
report the GPU path as passing.

## Voxel Buffer Terminology

Use these terms consistently in compute shader code, docs, comments, and buffer names:
